...
\begin{quote}
	...
	\begin{quote}
		...
		\begin{quote}
			Die zu regelnde Strecke $G_\omega^{'}$ weist zwei Pole auf der negativen Reellen achse auf und zwar bei \vspace{1em}
			$pol_{G\omega^{'}}_1 = - 0,0178492$ \vspace{1em}
			$pol_{G\omega^{'}}_2 = - 202.22547$\vspace{1em}
		...
		\begin{quote} 
		      ...
		      \cite{Ueberschwingweite}\vspace{1em}
		...
		\begin{quote}
			\begin{equation*}
            	\begin{split}
            		\overline{G}_{m\omega} (s) = \frac{\Omega(s)}{M_L (s)} = \frac{G_\omega^{'}}{1+ G_\omega^{'} T_\omega}
            	\end{split}
            \end{equation*}
            ...
            Es fällt auf, dass die Störsprungantwort extram langsam ist und daher vermultich nicht zu gebrauchen.Selbst
            nach über $150$s hat der Regler es nicht geschafft die Störung auszuregeln.\vspace{1em}
             
            Beim Vergleichen der Polstellen von $\overline{T}_\omega$ und $\overline{G}_\omega$ fällt es auf, dass sie
            nicht gleich viele haben. $\overline{T}_\omega$ hat 6 Polstellen während $\overline{G}_\omega$ 5 Polstellen hat.
		...
		\begin{quote}
			Nun verschieben wir die Nullstelle weiter in den Negativen bereich. Dadurch haben jetzt das neue $T_\omega$ und das
			neue $G_{m\omega}$ genau gleich viele Pole. Das liegt daran, dass nicht mehr automatisch bei $T_\omega$ ein Pol
			eliminiert wird, der bei $G_{m\omega}$ erhalten bleibt.\\
			Da wir die Nullstelle $s_{0\omega}$ auf die Stelle $-12$ gelegt haben und mit einer Verstärkung von $\frac{1}{45}$
			eingestellt haben, hat das positive Auswirkungen auf die Störübertragungsfunktion.
